...
const GENERIC_LANGUAGE_VALIDATION_SPECS = Object.freeze({
  go: {
    ...
    checkupCases: [
      ...
      {
        id: 'go:checkup:preserve_import_block',
        relativeFile: path.join('src', 'billing_import_block.go'),
        content: [
          'package billing',
          '',
          'import (',
          ...
          ')',
          '',
          'func Soma(valor float64) float64 {',
          '  return math.Max(valorr, 1)',
          '}',
        ].join('\n'),
        expectedKinds: ['undefined_variable'],
        expectedSnippetIncludes: ['return math.Max(valor, 1)'],
        forbiddenSnippetIncludes: ['"valor"', 'import valor'],
        applyKinds: ['undefined_variable'],
        mustClearKinds: ['undefined_variable'],
        expectedSourceIncludesAfterApply: ['  "math"', 'return math.Max(valor, 1)'],
      },
    ],
  },
  rust: {
    ...
    checkupCases: [
      ...
      {
        id: 'rust:checkup:preserve_use_block',
        relativeFile: path.join('src', 'billing_use_block.rs'),
        content: [
          'use std::cmp;',
          '',
          'pub fn soma(valor: i32) -> i32 {',
        ...
        expectedKinds: ['undefined_variable'],
        expectedSnippetIncludes: ['cmp::max(valor, 1)'],
        forbiddenSnippetIncludes: ['use valor', 'use std::valor'],
        applyKinds: ['undefined_variable'],
        mustClearKinds: ['undefined_variable'],
        expectedSourceIncludesAfterApply: ['use std::cmp;', 'cmp::max(valor, 1)'],
      },
    ],
  },
  ruby: {
  ...
  c: {
    ...
    checkupCases: [
      ...
      {
        id: 'c:checkup:preserve_include',
        relativeFile: path.join('src', 'billing_include.c'),
        content: [
          '#include <math.h>',
          '',
          'double soma(double valor) {',
          '  return fmax(valorr, 1);',
          '}',
        ].join('\n'),
        expectedKinds: ['undefined_variable'],
        expectedSnippetIncludes: ['return fmax(valor, 1);'],
        forbiddenSnippetIncludes: ['#include <valor.h>', '#include <valor>'],
        applyKinds: ['undefined_variable'],
        mustClearKinds: ['undefined_variable'],
        expectedSourceIncludesAfterApply: ['#include <math.h>', 'return fmax(valor, 1);'],
      },
    ],
  },
  terraform: {
