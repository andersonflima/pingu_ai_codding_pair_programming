...
function validateFileEntries(fileEntries) {
  ...
  try {
    return entries.flatMap((entry, index) => {
      ...
      if (result.status === 0) {
        return storeCachedValidationFailures(cacheKey, []);
      }
...
function classifyAutofixBatch(appliedIssues, fileEntries = []) {
  ...
  return {
    ...
    strategies: Array.from(new Set([
      ...
      !documentationOnly && !hasStructuralGeneration && !hasConsultativeOnly ? 'code_rewrite' : '',
    ].filter(Boolean))),
    strategy,
    documentationOnly,
    hasStructuralGeneration,
    hasConsultativeOnly,
    requiresRuntimeValidation: !documentationOnly && !hasConsultativeOnly,
