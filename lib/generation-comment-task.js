...
function createCommentTaskTools(deps) {
  ...
  function checkCommentTask(lines, file) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (marker === '**') {
        ...
        if (requiresAiForFeature(file, 'context_file') && !hasConfiguredAiCommand()) {
          issues.push(buildAiRequiredIssue(file, idx + 1, 'context_file'));
          return;
        }
        issues.push(...buildContextBlueprintTasks(lines, file, idx + 1, instruction));
        return;
      }
      if (requiresAiForFeature(file, 'comment_task') && !hasConfiguredAiCommand()) {
        issues.push(buildAiRequiredIssue(file, idx + 1, 'comment_task'));
        return;
      }
      ...
      issues.push({
        ...
        action: generatedTask.action || undefined,
  ...
  function buildAiRequiredIssue(file, line, feature) {
    return {
      file,
      line,
      severity: 'error',
      kind: 'ai_required',
      message: `IA obrigatoria para ${feature} em ${analysisExtension(file)}`,
      suggestion: 'Configure realtimeDevAgent.commentTaskAiCommand para habilitar o fluxo de IA do agente.',
      snippet: '',
