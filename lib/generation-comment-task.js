...
function createCommentTaskTools(deps) {
  ...
  function matchCommentTask(line, ext) {
    ...
    return matchers.reduce((resolved, matcher) => {
      ...
      if (matcher.marker) {
        return {
          marker: normalizeActionCommentMarker(matcher.marker),
          instruction: String(match[1] || '').trim(),
        };
      }

      return {
        marker: normalizeActionCommentMarker(match[1]),
        instruction: String(match[2] || '').trim(),
      };
    }, null);
  }

  function buildTerminalTask(lines, file, lineNumber, instruction, ext, strictAiCommentAction) {
  ...
  function checkCommentTask(lines, file, opts = {}) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (marker === '*') {
        ...
        const terminalTask = buildTerminalTask(lines, file, idx + 1, instruction, ext, strictAiCommentAction);
        if (terminalTask) {
