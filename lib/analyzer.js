...
function checkUndefinedVariables(lines, file) {
  ...
  lines.forEach((rawLine, idx) => {
    ...
    if (state.inFunction) {
      checkUndefinedLineInScope(rawLine, line, idx + 1, state, file, issues);
    } else {
...
function checkUndefinedLineInScope(rawLine, line, idx, state, file, issues) {
  ...
  const unknowns = extractUnknownVariables(line, candidates, assignmentSet);
...
function extractUnknownVariables(line, vars, assignmentSet) {
  const unknowns = [];
  for (const match of line.matchAll(/\b[a-z_][a-zA-Z0-9_?!]*\b/g)) {
    const token = match[0];
    if (tokenShouldIgnore(token, match.index, line, vars, assignmentSet)) {
      continue;
    }
...
function tokenShouldIgnore(token, start, line, vars, assignmentSet) {
  const len = token.length;
  const nextChar = start + len < line.length ? line[start + len] : '';
  if (isReservedToken(token) || /^[A-Z][A-Za-z0-9_?!]*$/.test(token) || nextChar === ':' || vars.has(token) || assignmentSet.has(token)) {
    return true;
  }
...
function checkCrossLanguageFunctionDocs(lines, file) {
  ...
  lines.forEach((line, idx) => {
    ...
    issues.push({
      ...
      snippet: buildLeadingFunctionDocumentation(declaration.name, declaration.params, declaration.name, ext),
    });
  });
  return issues;
}
function parseCrossLanguageFunctionDeclaration(line, ext) {
  ...
  if (isJavaScriptLikeExtension(lowerExt)) {
    match = String(line).match(/^\s*(?:export\s+)?(?:async\s+)?function\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)/);
  } else if (isPythonLikeExtension(lowerExt)) {
    match = String(line).match(/^\s*def\s+([a-z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*:/);
  } else if (isGoExtension(lowerExt)) {
    match = String(line).match(/^\s*func\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)/);
  } else if (isRustExtension(lowerExt)) {
    match = String(line).match(/^\s*(?:pub\s+)?(?:async\s+)?fn\s+([a-z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)/);
  } else if (lowerExt === '.rb') {
  ...
  return {
    ...
    params: parseGenericFunctionParams(match[2] || '', lowerExt),
  };
}
...
function isStructuredTextKind(kind) {
  return ['.yaml', '.yml', '.md', '.mmd', '.mermaid', '.tf', '.dockerfile'].includes(kind);
}
...
function scanSyntaxStructure(lines, kind) {
  ...
  const codeKindsWithSlashComments = new Set(['.js', '.jsx', '.ts', '.tsx', '.go', '.java', '.kts', '.kt', '.cs', '.c', '.cpp', '.h', '.hpp', '.rs', '.scala', '.swift']);
  ...
  lines.forEach((rawLine, index) => {
    ...
    for (let cursor = 0; cursor < line.length; cursor += 1) {
      ...
      if (['.py', '.ex', '.exs'].includes(kind) && (line.slice(cursor, cursor + 3) === '"""' || line.slice(cursor, cursor + 3) === "'''")) {
      ...
      if (codeKindsWithSlashComments.has(kind) && current === '/' && next === '*') {
        inBlockComment = true;
        cursor += 1;
        continue;
      }
...
function startsInlineComment(line, cursor, kind) {
  ...
  if (['.js', '.jsx', '.ts', '.tsx', '.go', '.java', '.kts', '.kt', '.cs', '.c', '.cpp', '.h', '.hpp', '.rs', '.scala', '.swift'].includes(kind)) {
    return current === '/' && next === '/';
  }
  if (['.ex', '.exs', '.rb', '.py', '.sh', '.toml', '.yaml', '.yml', '.tf', '.dockerfile'].includes(kind)) {
    return current === '#';
  }
  if (kind === '.lua') {
    return current === '-' && next === '-';
