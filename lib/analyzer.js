...
function analyzeText(filePath, text, opts = {}) {
  ...
  const aiResolvedIssues = activeLanguage
    ? resolveAutomaticIssuesWithAi(lines, analyzedFile, issues, {
      allowAiCalls: !skipAutomaticAiResolution,
    })
    : issues;
...
function parseCrossLanguageFunctionDeclaration(line, ext) {
  ...
  if (isJavaScriptLikeExtension(lowerExt)) {
    ...
    match = String(line).match(/^\s*(?:static\s+|inline\s+|extern\s+|const\s+|unsigned\s+|signed\s+|struct\s+\w+\s+|enum\s+\w+\s+|union\s+\w+\s+|volatile\s+|register\s+|long\s+|short\s+)*([A-Za-z_][A-Za-z0-9_]*)\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^;{}]*)\)\s*\{/);
    if (match && match[1] && match[2]) {
      returnAnnotation = String(match[1] || '').trim();
      rawName = String(match[2] || '').trim();
      rawParams = String(match[3] || '').trim();
    }
