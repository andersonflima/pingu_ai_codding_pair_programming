...
function checkDebugOutputs(lines, file) {
  ...
  const pattern = isJavaScriptLikeExtension(ext)
    ? /\b(?:console\.(?:log|debug|info|warn|error)|dbg)\s*\(/
    : /\b(?:IO\.puts|IO\.inspect|dbg)\b/;
  lines.forEach((line, idx) => {
...
function checkUndefinedVariables(lines, file) {
  ...
  if (isPythonLikeExtension(ext)) {
    return checkPythonUndefinedVariables(lines, file);
  }
...
function checkPythonUndefinedVariables(lines, file) {
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (!state.inFunction) {
      const declaration = parseCrossLanguageFunctionDeclaration(rawLine, '.py');
      if (!declaration) {
        continue;
      }
      const baseIndent = leadingIndentLength(rawLine);
      const params = (declaration.params || []).filter((param) => param !== 'self');
      state.inFunction = true;
      state.baseIndent = baseIndent;
      state.vars = new Set(params);
      state.allVars = collectPythonFunctionAllVariables(lines, idx, params, baseIndent);
      state.warned = new Set();
      continue;
    }
...
function collectPythonFunctionAllVariables(lines, startIdx, params, baseIndent) {
...
function extractPythonAssignmentVars(line) {
  ...
  const names = new Set();
  [...source.matchAll(/\b([a-z_][a-zA-Z0-9_]*)\s*=\s*(?![=<>~])/g)].forEach((match) => names.add(match[1]));
  [...source.matchAll(/\bfor\s+([a-z_][a-zA-Z0-9_]*)\s+in\b/g)].forEach((match) => names.add(match[1]));
...
function buildUndefinedVariableCorrectionSnippet(rawLine, unknown, suggestion, ext) {
  ...
  const correctedLine = replaceIdentifierOnce(sourceLine, unknown, suggestion);
  const indent = lineIndentation(sourceLine);
  const prefix = commentPrefix(ext);
  const correctionComment = `${indent}${prefix} pingu - correction : corrigido nome da variavel ${unknown} para ${suggestion}, pois ${unknown} nao existe no escopo atual.`;
