...
function analyzeText(filePath, text, opts = {}) {
  ...
  if (isStructuredTextKind(analyzedKind)) {
    ...
    issues.push(
      ...
      ...checkUndefinedVariables(lines, analyzedFile, { localModuleCache, focusRange, analysisMode }),
...
function checkUndefinedVariables(lines, file, opts = {}) {
  ...
  if (isPythonLikeExtension(ext)) {
    return importIssues.concat(
      filterIssuesByFocusRange(
        checkPythonUndefinedVariables(lines, file),
        focusRange,
      ),
    );
  }
  if (supportsBraceScopedUndefinedVariableAnalysis(ext)) {
    return importIssues.concat(
      filterIssuesByFocusRange(
        checkBraceScopedUndefinedVariables(lines, file, ext),
        focusRange,
      ),
    );
  }
