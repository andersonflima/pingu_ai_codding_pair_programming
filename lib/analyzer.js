...
function describeJavaScriptVariableTarget(candidate, insideClass, options = {}) {
  ...
  if (/\b(state|runtime|cache|store)\b/.test(`${name} ${combined}`)) {
    return projectEntity
      ? `Mantem o estado compartilhado usado pelo fluxo de ${projectEntity}.`
      : 'Mantem o estado compartilhado usado pelo fluxo atual.';
  }
  if (/\b(payload|message|event|response|request)\b/.test(`${name} ${combined}`)) {
    return 'Agrupa os dados principais usados pela proxima etapa do fluxo.';
  }
  if (insideClass) {
    return `Mantem ${name.replace(/_/g, ' ')} como parte do contrato interno da classe.`;
  }
  return `Prepara ${name.replace(/_/g, ' ')} para a proxima etapa do fluxo.`;
}
