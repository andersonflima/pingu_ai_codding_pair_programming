...
function parseJavaScriptLocalImportBindings(line) {
  ...
  const namedImportMatch = sourceLine.match(/^\s*import\s*\{([^}]+)\}\s*from\s*['"]([^'"]+)['"]/);
  if (namedImportMatch && isRelativeModuleSpecifier(namedImportMatch[2])) {
    return {
  ...
  const requireMatch = sourceLine.match(/^\s*(?:const|let|var)\s*\{([^}]+)\}\s*=\s*require\(\s*['"]([^'"]+)['"]\s*\)/);
  if (requireMatch && isRelativeModuleSpecifier(requireMatch[2])) {
    return {
...
function extractPythonImportVars(line) {
  ...
  if (directImport && directImport[1]) {
    splitTopLevelParams(directImport[1]).forEach((token) => {
      ...
      const rootName = sanitizeIdentifier(importToken.split('.')[0] || '');
      if (rootName) {
        names.add(rootName);
      }
    });
  ...
  splitTopLevelParams(fromImport[1]).forEach((token) => {
    ...
    const normalized = sanitizeIdentifier(importToken);
    if (normalized) {
...
function checkClassDocs(lines, file) {
  ...
  lines.forEach((line, idx) => {
    ...
    issues.push({
      ...
      severity: 'info',
      kind: 'class_doc',
      message: 'Classe sem documentacao',
