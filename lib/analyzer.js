...
function analyzeText(filePath, text, opts = {}) {
  ...
  if (isStructuredTextKind(analyzedKind)) {
    ...
    issues.push(
      ...
      ...checkTrailingWhitespace(lines, analyzedFile),
      ...checkTabs(lines, analyzedFile),
      ...(shouldAnalyzeDocumentation ? checkFunctionDocs(lines, analyzedFile) : []),
      ...(shouldAnalyzeDocumentation ? checkCrossLanguageFunctionDocs(lines, analyzedFile) : []),
      ...(shouldAnalyzeDocumentation ? checkClassDocs(lines, analyzedFile) : []),
      ...(shouldAnalyzeDocumentation && runLightAnalysisDeepPass ? checkVariableDocs(lines, analyzedFile) : []),
      ...(shouldAnalyzeDocumentation && runLightAnalysisDeepPass ? checkFlowMaintenanceComments(lines, analyzedFile) : []),
      ...(runLightAnalysisDeepPass ? checkFunctionSpecs(lines, analyzedFile) : []),
