...
function analyzeText(filePath, text, opts = {}) {
  ...
  if (isStructuredTextKind(analyzedKind)) {
    ...
    issues.push(
      ...
      ...checkUndefinedVariables(lines, analyzedFile, { localModuleCache }),
      ...
      ...checkClassDocs(lines, analyzedFile),
      ...checkVariableDocs(lines, analyzedFile),
  ...
  const sortedIssues = aiResolvedIssues
    .map((issue) => ({
      ...issue,
...
function checkUndefinedVariables(lines, file, opts = {}) {
  const ext = path.extname(file).toLowerCase();
  const importIssues = checkLocalImportBindings(lines, file, ext, opts);
  if (isElixirExtension(ext)) {
    return importIssues.concat(checkElixirUndefinedVariables(lines, file));
  }
  if (isPythonLikeExtension(ext)) {
    return importIssues.concat(checkPythonUndefinedVariables(lines, file));
  }
  if (supportsBraceScopedUndefinedVariableAnalysis(ext)) {
    return importIssues.concat(checkBraceScopedUndefinedVariables(lines, file, ext));
  }
  return importIssues;
}
...
function resolveLocalModuleExportNames(file, importSource, ext, cache) {
  ...
  const normalizedExportNames = Array.from(new Set(exportNames
    ...
    .filter(Boolean)));
  cache.set(exportCacheKey, normalizedExportNames);
  return normalizedExportNames;
}
...
function parsePythonFunctionDeclarationSource(source) {
  ...
  const match = normalized.match(/^def\s+([a-z_][a-zA-Z0-9_]*)\s*\((.*)\)\s*(?:->\s*([^:]+))?\s*:/);
  if (!match || !match[1]) {
    return null;
  }
...
function checkCrossLanguageFunctionDocs(lines, file) {
  ...
  if (isPythonLikeExtension(ext)) {
    return checkPythonFunctionDocs(lines, file);
  }
...
function checkPythonFunctionDocs(lines, file) {
  const issues = [];
...
function checkClassDocs(lines, file) {
  const ext = path.extname(file).toLowerCase();
  if (!isPythonLikeExtension(ext)) {
    return [];
  }
  ...
  lines.forEach((line, idx) => {
    ...
    issues.push({
      ...
      snippet: [
        `${indent}"""`,
        `${indent}Representa a responsabilidade principal de ${className}.`,
        `${indent}"""`,
      ].join('\n'),
...
function checkVariableDocs(lines, file) {
  const ext = path.extname(file).toLowerCase();
  if (!isPythonLikeExtension(ext)) {
    return [];
  }

  return checkPythonVariableDocs(lines, file);
}
function checkPythonVariableDocs(lines, file) {
  const issues = [];
  let multilineQuote = '';
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    const className = parsePythonClassDeclaration(rawLine);
    if (className) {
      issues.push(...collectPythonClassVariableDocIssues(lines, file, idx));
...
function collectPythonClassVariableDocIssues(lines, file, classStartIdx) {
...
function shouldSkipPythonVariableDocCandidate(candidate, insideClass) {
  ...
  return trivialScalarNames.has(normalizedName) && trivialScalarTypes.has(annotationType);
}
...
function buildPythonVariableDocIssue(file, idx, rawLine, candidate, insideClass, options = {}) {
  ...
  return {
    ...
    snippet: buildPythonVariableDocumentationSnippet(rawLine, candidate, insideClass),
    metadata: {
...
function buildPythonVariableDocumentationSnippet(rawLine, candidate, insideClass) {
  if (insideClass || candidate.style === 'annotation' || /^[A-Z][A-Za-z0-9_]*$/.test(String(candidate && candidate.name || ''))) {
    return `${lineIndentation(rawLine)}# ${describePythonVariableTarget(candidate, insideClass)}`;
  }
...
function describePythonVariableTarget(candidate, insideClass) {
