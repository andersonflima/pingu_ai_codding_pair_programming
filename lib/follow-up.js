...
function buildFollowUpInstruction(issue) {
  ...
  if (kind === 'function_doc') {
    ...
    if (ext === '.py') {
      return 'adicione docstring Python idiomatica no formato Google explicando responsabilidade, argumentos e retorno com base no corpo da funcao, sem repetir o nome do simbolo nem inventar comportamento';
    }
    if (['.js', '.jsx', '.ts', '.tsx', '.mjs', '.cjs'].includes(ext)) {
      return 'adicione JSDoc idiomatico explicando responsabilidade, parametros e retorno com base no codigo real, sem comentario tautologico';
    }
    return 'adicione @doc idiomatico para a funcao publica mantendo a regra de negocio';
  }
  ...
  if (kind === 'class_doc') {
    if (ext === '.py') {
      return 'adicione docstring curta e idiomatica para a classe deixando clara a responsabilidade principal e o contrato da estrutura';
    }
    return 'adicione documentacao curta para a classe mantendo o contrato atual';
  }

  if (kind === 'variable_doc') {
    return 'adicione comentario curto e contextual para a variavel ou atributo explicando o papel no fluxo ou contrato, sem apenas repetir nome e tipo';
  }
  ...
  if (kind === 'flow_comment') {
    return 'adicione comentario curto e contextual acima deste passo explicando por que ele existe e qual efeito prepara no fluxo atual';
  }
