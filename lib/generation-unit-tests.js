...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function checkUnitTestCoverage(lines, file) {
    ...
    if (requiresAiForFeature && requiresAiForFeature(file, 'unit_test') && hasConfiguredAiCommand && !hasConfiguredAiCommand()) {
      return [{
        file,
        line: 1,
        severity: 'error',
        kind: 'ai_required',
        message: 'IA obrigatoria para gerar testes unitarios neste arquivo',
        suggestion: 'Configure realtimeDevAgent.commentTaskAiCommand para habilitar geracao automatica de testes.',
        snippet: '',
    ...
    if (requiresAiForFeature && requiresAiForFeature(file, 'unit_test')) {
      const aiGeneratedUnitTests = resolveAiGeneratedUnitTests
        ? resolveAiGeneratedUnitTests({
          ...
          activeBlueprint: typeof loadActiveBlueprintContext === 'function'
            ? loadActiveBlueprintContext(file)
            : null,
          existingTestContent: pathExists(targetFile) ? fs.readFileSync(targetFile, 'utf8') : '',
      ...
      if (!aiGeneratedUnitTests || !String(aiGeneratedUnitTests.snippet || '').trim()) {
        return [{
          ...
          severity: 'error',
          kind: 'ai_required',
          message: 'IA obrigatoria para gerar testes unitarios completos neste arquivo',
          suggestion: 'Garanta que realtimeDevAgent.commentTaskAiCommand esteja configurado e responda com o arquivo de testes.',
          snippet: '',
          action: { op: 'insert_before' },
        }];
      }
  ...
  function buildUnitTestIssue(file, line, targetFile, message, suggestion, snippet, actionOverride = null) {
    return {
      ...
      suggestion,
      snippet,
      action: actionOverride && typeof actionOverride === 'object'
        ? {
          ...actionOverride,
