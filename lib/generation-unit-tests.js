...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function resolveUnitTestTargetFile(file, ext) {
    ...
    if (isGoExtension(lowerExt)) {
      return resolveGoImportPath(file)
        ? path.join(projectRoot, 'tests', sourceDir, `${baseName}_test.go`)
        : path.join(projectRoot, 'tests', sourceDir, `${baseName}_test.sh`);
    }
    if (isRustExtension(lowerExt)) {
      return resolveCargoPackageName(file)
        ? path.join(projectRoot, 'tests', sourceDir, `${baseName}_test.rs`)
        : path.join(projectRoot, 'tests', sourceDir, `${baseName}_test.sh`);
    }
    if (isRubyExtension(lowerExt)) {
    ...
    if (isCExtension(lowerExt)) {
      return path.join(projectRoot, 'tests', sourceDir, `${baseName}_test.c`);
    }
    if (lowerExt === '.lua') {
    ...
    if (lowerExt === '.dockerfile' || isComposeFile(file) || lowerExt === '.md' || isMermaidExtension(lowerExt)) {
      return path.join(projectRoot, 'tests', sourceDir, `${baseName}_test.sh`);
    }
  ...
  function resolveSupplementalUnitTestTargetFile(file, ext, candidateName) {
    ...
    if (isGoExtension(lowerExt)) {
      return resolveGoImportPath(file)
        ? path.join(projectRoot, 'tests', sourceDir, `${baseName}_${safeCandidateName}_test.go`)
        : '';
    }
    if (isRustExtension(lowerExt)) {
      return resolveCargoPackageName(file)
        ? path.join(projectRoot, 'tests', sourceDir, `${baseName}_${safeCandidateName}_test.rs`)
        : '';
    }
    if (isRubyExtension(lowerExt)) {
    ...
    if (isCExtension(lowerExt)) {
      return path.join(projectRoot, 'tests', sourceDir, `${baseName}_${safeCandidateName}_test.c`);
    }
    if (lowerExt === '.lua') {
