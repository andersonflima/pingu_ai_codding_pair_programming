...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function checkUnitTestCoverage(lines, file) {
    ...
    if (requiresAiForFeature && requiresAiForFeature(file, 'unit_test') && hasOpenAiConfiguration && !hasOpenAiConfiguration()) {
      return [{
        file,
        line: 1,
        severity: 'error',
        ...
        suggestion: 'Configure OPENAI_API_KEY para habilitar geracao automatica de testes com OpenAI Codex.',
        snippet: '',
    ...
    if (requiresAiForFeature && requiresAiForFeature(file, 'unit_test')) {
      ...
      if (!aiGeneratedUnitTests || !String(aiGeneratedUnitTests.snippet || '').trim()) {
        return [{
          ...
          suggestion: 'Garanta que OPENAI_API_KEY esteja configurada para o OpenAI Codex responder com o arquivo de testes.',
          snippet: '',
          action: { op: 'insert_before' },
        }];
      }
