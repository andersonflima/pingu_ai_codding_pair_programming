...
const { fixPriorityForKind } = require('./issue-kinds');
...
function selectAutomaticAiCommentIssueKeys(issues = [], budget = DEFAULT_AUTOMATIC_AI_COMMENT_MAX_ISSUES) {
  ...
  return new Set(
    commentIssues
      .sort((left, right) => {
        const leftPriority = fixPriorityForKind(left && left.kind);
        const rightPriority = fixPriorityForKind(right && right.kind);
