...
function resolveAutomaticIssuesWithAi(lines, file, issues = []) {
  ...
  const contextIssues = detectElixirContextContractIssues(lines, file, issues);
  const mergedIssues = [...issues, ...contextIssues];

  return mergedIssues.map((issue) => {
    if (!issue || !ELIXIR_AI_AUTOFIX_KINDS.has(String(issue.kind || ''))) {
      return issue;
    }
...
function detectElixirContextContractIssues(lines, file, existingIssues = []) {
  ...
  if (!/\b(calculadora|calculator|calculo|calcular|operacao|resultado)\b/.test(contextSignal)) {
    return [];
  }
  ...
  if (existingIssues.some((issue) => issue.kind === 'context_contract')) {
    return [];
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!isCalculatorOutputFunctionName(declaration.name)) {
      continue;
    }
    ...
    const lastMeaningful = lastMeaningfulElixirBodyEntry(block.body);
    if (!lastMeaningful) {
      continue;
    }
    if (!/^\s*(true|false)\s*$/.test(lastMeaningful.text)) {
      continue;
    }

    const preferredReturnExpression = inferNumericReturnExpression(block.body);
...
function isCalculatorOutputFunctionName(name) {
  return /\b(resultado|result|total|calcular|soma|somar|subtrair|subtracao|multiplicar|multiplicacao|dividir|divisao)\b/i.test(String(name || ''));
}
...
function lastMeaningfulElixirBodyEntry(bodyEntries) {
...
function inferNumericReturnExpression(bodyEntries) {
  for (let index = bodyEntries.length - 1; index >= 0; index -= 1) {
