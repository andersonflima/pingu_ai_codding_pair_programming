...
function synthesizeFromCommentTask(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  const shellContextualUpdateTarget = resolveShellContextualUpdateTarget(
    instruction,
    normalizedExt,
    lines,
    sourceFile,
  ...
  const effectiveInstruction = shellContextualUpdateTarget
    ? buildShellContextualUpdateInstruction(instruction, shellContextualUpdateTarget)
  ...
  const aiGeneratedTask = resolveAiGeneratedTask({
    ...
    semanticIntent,
    intentIR,
    targetContext: shellContextualUpdateTarget || undefined,
  });
  const validatedAiTask = validateAiGeneratedTask(aiGeneratedTask, ext, semanticIntent);
  if (validatedAiTask) {
    const normalizedAiTask = shellContextualUpdateTarget
      ? materializeShellContextualUpdateTask(validatedAiTask, shellContextualUpdateTarget, lines, sourceFile)
      : validatedAiTask;
    if (normalizedAiTask) {
  ...
  if (shellContextualUpdateTarget) {
    return {
      snippet: '',
      aiFailure: true,
      aiFailureMessage: `Atualizacao contextual da funcao shell '${shellContextualUpdateTarget.name}' exige uma resposta de IA valida; o fallback offline foi bloqueado para evitar gerar uma funcao nova incorreta.`,
      semanticIntent,
      intentIR,
    };
  }
...
function isShellContextualUpdateInstruction(instruction) {
...
function findShellFunctionRange(lines, startSearchIndex = 0, expectedName = '') {
  if (!Array.isArray(lines) || lines.length === 0) {
    return null;
  }

  const normalizedExpectedName = String(expectedName || '').trim()
    ? sanitizeIdentifier(expectedName)
    : '';
  let functionStart = -1;
  let functionName = '';

  for (let index = Math.max(0, startSearchIndex); index < lines.length; index += 1) {
    const match = String(lines[index] || '').match(/^\s*(?:function\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*(?:\(\s*\))?\s*\{\s*$/);
    if (!match || !match[1]) {
      continue;
    }
    const candidateName = sanitizeIdentifier(match[1]);
    if (normalizedExpectedName && candidateName !== normalizedExpectedName) {
      continue;
    }
    functionStart = index;
  ...
  for (let index = functionStart; index < lines.length; index += 1) {
    ...
    const opens = (sourceLine.match(/\{/g) || []).length;
    const closes = (sourceLine.match(/\}/g) || []).length;
    depth += opens - closes;
...
function findShellFunctionRangeBelow(lines, lineIndex) {
  const startSearchIndex = Number.isInteger(lineIndex) && lineIndex >= 0 ? lineIndex + 1 : 0;
  return findShellFunctionRange(lines, startSearchIndex);
}

function resolveShellContextualUpdateTarget(instruction, ext, lines = [], sourceFile = '', options = {}) {
  if (!isShellExtension(ext) || !isShellContextualUpdateInstruction(instruction) || !sourceFile) {
    return null;
  }

  const commentIndex = Number.isInteger(options.lineIndex) ? options.lineIndex : -1;
  const functionRange = findShellFunctionRangeBelow(lines, commentIndex);
  if (!functionRange) {
    return null;
  }
...
function buildShellContextualUpdateInstruction(instruction, target) {
  const functionName = String(target && target.name || '').trim() || 'funcao_alvo';
  const requestedChange = safeComment(instruction);
  return [
    `Atualize somente a funcao shell ${functionName}.`,
...
function extractShellFunctionSnippet(snippet, functionName = '') {
  ...
  const directRange = findShellFunctionRange(lines, 0, functionName);
  if (directRange) {
    return lines.slice(directRange.start, directRange.end + 1).join('\n').trim();
  }

  const firstLine = String(lines[0] || '').trim();
  if (/^(?:function\s+)?[A-Za-z_][A-Za-z0-9_]*\s*(?:\(\s*\))?\s*\{\s*$/.test(firstLine)) {
    return source;
  }
...
function materializeShellContextualUpdateTask(generatedTask, target, lines = [], sourceFile = '') {
  if (!target || !sourceFile) {
    return null;
  }

  const updatedFunctionSnippet = extractShellFunctionSnippet(
...
function buildShellContextualUpdateAiFailure(instruction, lines = [], options = {}) {
  if (!isShellContextualUpdateInstruction(instruction)) {
    return null;
  }

  const functionRange = findShellFunctionRangeBelow(
    lines,
    Number.isInteger(options.lineIndex) ? options.lineIndex : -1,
  );
  if (!functionRange) {
    return null;
  }
  ...
  return {
    snippet: '',
    aiFailure: true,
    aiFailureMessage: `Atualizacao contextual da funcao shell '${functionRange.name}' exige uma resposta de IA valida; o fallback offline foi bloqueado para evitar gerar uma funcao nova incorreta.`,
  };
}
function generateFunctionSnippet(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  if (isShellExtension(ext)) {
    const shellContextualUpdate = buildShellContextualUpdateAiFailure(instruction, lines, options);
    if (shellContextualUpdate) {
      return shellContextualUpdate;
    }
...
function generateContextualCorrectionSnippet(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  if (isShellExtension(normalizedExt)) {
    return buildShellContextualUpdateAiFailure(instruction, lines, options);
  }
...
function parseGoPublicFunctionHeader(line) {
  const match = String(line || '').match(/^\s*func\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)/);
  if (!match) {
    return null;
  }
  return { name: String(match[1] || '') };
