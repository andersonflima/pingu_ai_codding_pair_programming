...
function generateShellScriptSnippet(instruction) {
  const explicitLiteral = extractLiteralFromInstruction(String(instruction || '').toLowerCase());
  const bodyLine = explicitLiteral
    ? `printf '%s\\n' ${explicitLiteral}`
    : `printf '%s\\n' ${JSON.stringify(`TODO: implementar script para: ${safeComment(instruction)}`)}`;
