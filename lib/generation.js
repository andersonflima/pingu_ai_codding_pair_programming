...
function mustUseAiForCommentAction() {
  return /^(?:1|true|yes|on)$/i.test(String(process.env.PINGU_FORCE_AI_COMMENT_TASK || '').trim());
}

function preferAiForCommentTask() {
  return /^(?:1|true|yes|on)$/i.test(String(process.env.PINGU_PREFER_AI_COMMENT_TASK || '').trim());
}
...
function synthesizeFromCommentTask(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  return {
    snippet: '',
    aiFailure: true,
    aiFailureMessage: `IA preferida nao retornou implementacao valida para comment_task em ${normalizedExt || 'arquivo atual'}.`,
    semanticIntent,
    intentIR,
  };
}
...
function automaticAiResolutionEnabled(env = process.env) {
  return /^(?:1|true|yes|on)$/i.test(String(env.PINGU_AUTOMATIC_AI_RESOLUTION || ''));
}
...
function resolveAutomaticIssuesWithAi(lines, file, issues = [], options = {}) {
  ...
  if (!automaticAiResolutionEnabled(env)) {
    if (DEBUG_AUTOMATIC_AI) {
  ...
  const resolved = mergedIssues.map((issue, index) => {
    ...
    if (!allowAiCalls) {
      return buildAutomaticAiRequiredIssue(
        file,
        issue,
        `IA obrigatoria para corrigir ${issue.kind} em ${normalizedExt}: modo atual bloqueou chamadas de IA`,
      );
    }
    ...
    if (!aiResolution || !String(aiResolution.snippet || '').trim()) {
      return buildAutomaticAiRequiredIssue(
        file,
        issue,
        `IA obrigatoria nao retornou snippet aplicavel para ${issue.kind} em ${normalizedExt}`,
      );
    }
    ...
    if (!isAutomaticAiSnippetCompatible(issue, resolvedAction, aiResolution.snippet, lines)) {
      return buildAutomaticAiRequiredIssue(
        file,
        issue,
        `IA obrigatoria retornou snippet/action incompativel para ${issue.kind} em ${normalizedExt}`,
      );
    }
