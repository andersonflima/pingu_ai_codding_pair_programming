  ...
  if (isPythonLikeExtension(lowerExt)) {
    return [
      ...
      ...(documentation.paramDocs.length
        ? documentation.paramDocs.map((param) => `#   ${param.name} (${param.pythonType}): ${param.description}`)
        : ['#   Nenhum argumento recebido.']),
      '# ---',
      '# Returns:',
      `#   ${documentation.pythonReturnType}: ${documentation.returnDescription}`,
...
function buildAutomaticAiRequiredIssue(file, issue, message = '') {
  return {
    ...
    suggestion: 'Configure realtimeDevAgent.commentTaskAiCommand para habilitar as correcoes automaticas orientadas por IA.',
    snippet: '',
