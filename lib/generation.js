...
function inferFunctionDocumentationContext(name, params, instruction, ext, context = {}) {
  ...
  const paramDocs = normalizedParams.map((param, index) => {
    const paramContext = Array.isArray(context.paramDescriptors) ? context.paramDescriptors[index] || {} : {};
    const semanticType = inferSemanticParamType(param, instruction, ext, {
      ...context,
      paramContext,
      semanticReturnType,
    });
    return {
      name: param,
      type: renderJavaScriptDocType(semanticType, paramContext.annotation || ''),
      pythonType: renderPythonDocType(semanticType, paramContext.annotation || ''),
      description: describeParameterForDocumentation(param, semanticType, context),
    };
  });
