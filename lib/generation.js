...
function synthesizeFromCommentTask(instruction, ext, lines = [], sourceFile = '') {
  ...
  if (classified === 'example') {
    return generateExampleSnippet(effectiveInstruction, ext);
  }
  if (classified === 'crud') {
    return generateCrudSnippet(effectiveInstruction, ext);
  }
  if (classified === 'ui') {
    return generateUiSnippet(effectiveInstruction, ext, lines);
  }
  if (classified === 'structure') {
    return generateStructureSnippet(effectiveInstruction, ext);
  }
  if (classified === 'function') {
    return generateFunctionSnippet(effectiveInstruction, ext, lines);
  }
  if (classified === 'comment') {
    return generateCommentSnippet(effectiveInstruction, ext);
  }
  if (classified === 'test') {
    return generateTestSnippet(effectiveInstruction, ext);
  }
  return generateGenericSnippet(effectiveInstruction, ext);
}
...
function isRubyExtension(ext) {
  return ['.rb'].includes(String(ext || '').toLowerCase());
}
...
function classifyCommentTask(instruction) {
  ...
  if (/\b(tela|pagina|página|screen|page|login|formulario|formulário|form|componente|component|modal|dashboard|dado|dice|rpg)\b/i.test(instruction)) {
    return 'ui';
  }
...
function findUntestedUnitTestCandidates(file, ext, targetFile, candidates) {
  ...
  return candidates.filter((candidate) => {
    return !relatedContents.some((content) => unitTestCandidateCovered(candidate, content, ext));
  });
}
