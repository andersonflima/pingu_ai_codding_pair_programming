...
function synthesizeFromCommentTask(instruction, ext, lines = [], sourceFile = '') {
  ...
  const renderedTask = renderCommentTaskByIntent(renderableIntentKind, effectiveInstruction, ext, lines, sourceFile);
  return finalizeGeneratedTaskResult(
...
function renderCommentTaskByIntent(intentKind, instruction, ext, lines = [], sourceFile = '') {
  ...
  const renderers = {
    ...
    generic: () => generateGenericSnippet(instruction, ext),
  };
