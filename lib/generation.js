...
function inferPreferredContextReturnExpression(bodyEntries, expectedContractKind, language) {
  ...
  return inferPreferredReturnExpressionFromJavaScript(bodyEntries, expectedContractKind);
}
...
function inferPreferredReturnExpressionFromJavaScript(bodyEntries, expectedContractKind) {
  for (let index = bodyEntries.length - 1; index >= 0; index -= 1) {
    const text = String(bodyEntries[index] && bodyEntries[index].text || '').trim();
    const candidate = parseJavaScriptContextContractCandidate(text);
    if (contextContractCandidateMatchesKind(candidate, expectedContractKind)) {
      return candidate.preferredExpression;
    }
  }
...
function parseJavaScriptContextContractCandidate(text) {
  const assignment = String(text || '').match(/^(?:(?:const|let|var)\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+?);?$/);
  if (assignment) {
    return {
      name: String(assignment[1] || '').trim(),
      expression: String(assignment[2] || '').trim(),
