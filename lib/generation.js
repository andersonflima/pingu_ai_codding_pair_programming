...
const { semanticPriorityForIssue } = require('./issue-confidence');
...
function synthesizeFromCommentTask(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  const strictAiCommentTask = requiresAiForFeature(sourceFile, 'comment_task');
  ...
  if (validatedAiTask) {
    return finalizeGeneratedTaskResult(
      withSemanticMetadata(validatedAiTask, semanticIntent, intentIR),
      ext,
      lines,
      sourceFile,
  ...
  if (strictAiCommentTask) {
    return {
      snippet: '',
      aiFailure: true,
      aiFailureMessage: 'IA obrigatoria nao retornou implementacao valida para comment_task em Elixir.',
      semanticIntent,
      intentIR,
    };
  }
...
function classifyCommentTask(instruction) {
  ...
  if (/\b(corrige|corrigir|corrigindo|ajusta|ajustar|substitui|substituir|altera|alterar|troca|trocar|remove|remover|troque|corrija|refatora|refatorar|refactor)\b/i.test(instruction)) {
    return 'generic';
  }
...
function inferFunctionDocumentationContext(name, params, instruction, ext, context = {}) {
  ...
  const summary = context.summary || functionDocumentationSummary(name, instruction);
  const semanticReturnType = inferSemanticReturnType(name, instruction, ext, context);
  const returnDescription = context.returnDescription || functionReturnDocumentation(instruction, ext, semanticReturnType);
...
function describeParameterForDocumentation(name, semanticType) {
  const normalizedName = String(name || '').toLowerCase();
  if (/^(room_?id|sala_?id|chat_?id)$/.test(normalizedName)) {
  ...
  switch (semanticType) {
    ...
    case 'array':
      return 'Colecao de entrada processada pelo fluxo principal.';
    case 'object':
      return 'Estrutura de dados de entrada consumida pela funcao.';
    default:
...
function functionReturnDocumentation(instruction, ext, semanticType = '') {
  const diceSides = extractDiceSides(instruction);
  if (diceSides) {
  ...
  if (semanticType === 'number') {
    return 'Valor numerico calculado conforme a regra principal da funcao.';
  }
  if (semanticType === 'string') {
    return 'Texto resultante produzido pela regra principal da funcao.';
  }
  if (semanticType === 'boolean') {
    return 'Resultado booleano que representa o desfecho da regra principal.';
  }
  if (semanticType === 'array') {
    return 'Colecao resultante produzida pelo fluxo principal da funcao.';
  }
  if (semanticType === 'object') {
    return 'Estrutura de dados retornada ao final do fluxo principal.';
  }
  if (semanticType === 'void') {
    return 'Nao retorna valor util para o chamador.';
  }
...
function generateGenericSnippet(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  if (/\b(corrige|corrigir|corret|ajusta|ajustar|substitui|substituir|remove|remover|fix|refatora|refatorar|refactor)\b/i.test(down)) {
...
function selectAutomaticAiCommentIssueKeys(issues = [], budget = DEFAULT_AUTOMATIC_AI_COMMENT_MAX_ISSUES) {
  ...
  return new Set(
    commentIssues
      .sort((left, right) => {
        const leftPriority = semanticPriorityForIssue(left);
        const rightPriority = semanticPriorityForIssue(right);
...
function prefersHeuristicCommentFallback(ext, issue) {
  ...
  if (isPythonLikeExtension(normalizedExt)) {
    return ['class_doc', 'flow_comment', 'function_doc', 'variable_doc'].includes(kind);
  }

  if (isJavaScriptLikeExtension(normalizedExt)) {
    return ['flow_comment', 'function_doc'].includes(kind);
  }

  return false;
}

function resolveAutomaticIssuesWithAi(lines, file, issues = []) {
...
function detectElixirContextContractIssues(lines, file, existingIssues = []) {
  if (!hasCalculatorContextSignal(file)) {
    return [];
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const declaration = parseElixirPublicFunctionHeader(header);
    if (!declaration || !isCalculatorOutputFunctionName(declaration.name)) {
      continue;
    }

    const block = collectElixirFunctionBlock(lines, index);
    if (!block || block.body.length === 0) {
      continue;
    }
    const lastMeaningful = lastMeaningfulBodyEntry(block.body, '#');
    if (!lastMeaningful || !/^\s*(true|false)\s*$/.test(lastMeaningful.text)) {
      continue;
    }

    const preferredReturnExpression = inferNumericReturnExpressionFromElixir(block.body);
    if (!preferredReturnExpression) {
      continue;
    }

    issues.push(buildContextContractIssue(file, index + 1, preferredReturnExpression));
  }
...
function detectJavaScriptContextContractIssues(lines, file, existingIssues = []) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const header = String(lines[index] || '');
    const declaration = parseJavaScriptPublicFunctionHeader(header);
    if (!declaration || !isCalculatorOutputFunctionName(declaration.name)) {
      continue;
    }

    const block = collectJavaScriptFunctionBlock(lines, index);
    if (!block || block.body.length === 0) {
      continue;
    }

    const lastMeaningful = lastMeaningfulBodyEntry(block.body, '//');
    if (!lastMeaningful || !/^\s*(?:return\s+)?(?:true|false)\s*;?\s*$/.test(lastMeaningful.text)) {
      continue;
    }

    const preferredReturnExpression = inferNumericReturnExpressionFromJavaScript(block.body);
    if (!preferredReturnExpression) {
      continue;
    }
...
function detectPythonContextContractIssues(lines, file, existingIssues = []) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const header = String(lines[index] || '');
    const declaration = parsePythonPublicFunctionHeader(header);
    if (!declaration || !isCalculatorOutputFunctionName(declaration.name)) {
      continue;
    }

    const block = collectPythonFunctionBlock(lines, index);
    if (!block || block.body.length === 0) {
      continue;
    }

    const lastMeaningful = lastMeaningfulBodyEntry(block.body, '#');
    if (!lastMeaningful || !/^\s*return\s+(?:True|False)\s*$/.test(lastMeaningful.text)) {
      continue;
    }

    const preferredReturnExpression = inferNumericReturnExpressionFromPython(block.body);
    if (!preferredReturnExpression) {
      continue;
    }
...
function hasCalculatorContextSignal(file) {
  ...
  const contextSignal = [
    String(activeBlueprint.entity || ''),
    String(activeBlueprint.summary || ''),
    String(activeBlueprint.body || ''),
    String(activeBlueprint.document || ''),
  ].join(' ').toLowerCase();
  return /\b(calculadora|calculator|calculo|calcular|operacao|resultado|sum|soma)\b/.test(contextSignal);
}
...
function buildContextContractIssue(file, line, preferredReturnExpression) {
  return {
    file,
    line,
    severity: 'warning',
    kind: 'context_contract',
    message: 'Retorno booleano conflita com o contexto ativo de calculadora',
    suggestion: 'Retorne o resultado numerico calculado para manter o contrato esperado pelo contexto.',
    snippet: '',
    action: {
      op: 'write_file',
      target_file: file,
    ...
    contextHint: {
      preferredReturnExpression,
    },
  };
}

function parseElixirPublicFunctionHeader(line) {
  const match = String(line || '').match(/^\s*def\s+([a-z_][a-zA-Z0-9_?!]*)\s*\(([^)]*)\)\s+do\b/);
  if (!match) {
    return null;
  }
  return {
...
function parseJavaScriptPublicFunctionHeader(line) {
  ...
  if (!match) {
    const methodMatch = source.match(/^\s*(?:async\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*\([^)]*\)\s*\{/);
...
function parsePythonPublicFunctionHeader(line) {
  const match = String(line || '').match(/^\s*def\s+([a-z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*:/);
  if (!match) {
    return null;
  }
  return {
...
function isCalculatorOutputFunctionName(name) {
  return /\b(resultado|result|total|calcular|calculate|soma|sum|somar|subtrair|subtracao|multiplicar|multiplicacao|dividir|divisao)\b/i.test(String(name || ''));
}
...
function inferNumericReturnExpressionFromElixir(bodyEntries) {
  for (let index = bodyEntries.length - 1; index >= 0; index -= 1) {
    const text = String(bodyEntries[index] && bodyEntries[index].text || '').trim();
    const assignment = text.match(/^([a-z_][a-zA-Z0-9_?!]*)\s*=\s*(.+)$/);
    if (!assignment) {
      continue;
    }
    const expression = String(assignment[2] || '');
    if (/[+\-*/]/.test(expression)) {
      return assignment[1];
    }
  }
...
function inferNumericReturnExpressionFromJavaScript(bodyEntries) {
  for (let index = bodyEntries.length - 1; index >= 0; index -= 1) {
    const text = String(bodyEntries[index] && bodyEntries[index].text || '').trim();
    const assignment = text.match(/^(?:const|let|var)\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+);?$/);
    if (assignment) {
      const expression = String(assignment[2] || '');
      if (/[+\-*/]/.test(expression)) {
        return assignment[1];
    ...
    const returnExpression = text.match(/^return\s+(.+);?$/);
    if (returnExpression && /[+\-*/]/.test(String(returnExpression[1] || ''))) {
      return String(returnExpression[1] || '').trim();
    }
  }
  return '';
}

function inferNumericReturnExpressionFromPython(bodyEntries) {
  for (let index = bodyEntries.length - 1; index >= 0; index -= 1) {
    const text = String(bodyEntries[index] && bodyEntries[index].text || '').trim();
    const assignment = text.match(/^([a-z_][a-zA-Z0-9_]*)\s*=\s*(.+)$/);
    if (assignment) {
