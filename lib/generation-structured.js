...
function createStructuredGenerators(helpers = {}) {
  ...
  function generateStructureSnippet(instruction, ext) {
    ...
    if (intent.kind === 'interface') {
      return {
        snippet: generateInterfaceSnippet(intent, ext),
        structuredIntent: intent,
        semanticIdentity: semanticIdentityForIntent(intent),
        disableMaintenanceComments: true,
      };
    ...
    if (intent.kind === 'struct') {
      return {
        snippet: generateStructSnippet(intent, ext),
        structuredIntent: intent,
        semanticIdentity: semanticIdentityForIntent(intent),
        disableMaintenanceComments: true,
      };
    ...
    if (intent.kind === 'module') {
      return {
        snippet: generateModuleSnippet(intent, ext),
        structuredIntent: intent,
        semanticIdentity: semanticIdentityForIntent(intent),
        disableMaintenanceComments: true,
      };
    ...
    if (intent.kind === 'object') {
      return {
        snippet: generateObjectSnippet(intent, ext),
        structuredIntent: intent,
        semanticIdentity: semanticIdentityForIntent(intent),
        disableMaintenanceComments: true,
      };
    ...
    if (intent.kind === 'collection') {
      return {
        snippet: variableDeclarationSnippet(intent.name, collectionLiteralForLanguage(intent.items, ext), ext),
        structuredIntent: intent,
        semanticIdentity: semanticIdentityForIntent(intent),
        disableMaintenanceComments: true,
      };
    ...
    if (intent.kind === 'variable') {
      ...
      return {
        snippet: variableDeclarationSnippet(intent.name, intent.value, ext),
        structuredIntent: intent,
        semanticIdentity: semanticIdentityForIntent(intent),
        disableMaintenanceComments: true,
      };
  ...
  function generateObjectSnippet(objectIntentOrInstruction, ext) {
    const lowerExt = String(ext || '').toLowerCase();
    const objectIntent = resolveObjectIntent(objectIntentOrInstruction);
    const objectName = objectIntent.name;
    const fields = objectIntent.fields;
  ...
  function resolveObjectIntent(objectIntentOrInstruction) {
    if (
      objectIntentOrInstruction
      && typeof objectIntentOrInstruction === 'object'
      && objectIntentOrInstruction.kind === 'object'
    ) {
      return {
        name: objectIntentOrInstruction.name || 'dados',
        fields: Array.isArray(objectIntentOrInstruction.fields)
          ? objectIntentOrInstruction.fields
          : [],
      };
    }

    return {
      name: inferObjectName(objectIntentOrInstruction),
      fields: inferObjectFields(objectIntentOrInstruction),
    };
  }
  ...
  function generateClassSnippet(classIntentOrInstruction, ext) {
    ...
    if (!hasFields && !hasMethods) {
      return minimalClassSnippet(className, lowerExt);
    }

    if (['.js', '.jsx', '.mjs', '.cjs'].includes(lowerExt)) {
      const constructorBlock = hasFields
        ? [
    ...
    if (isPythonLikeExtension(lowerExt)) {
      const constructorSignature = hasFields
        ? `    def __init__(self, ${fields.map((field) => `${field}=${pythonClassConstructorDefaultValue(field)}`).join(', ')}):`
        : '';
      const constructorBody = hasFields
        ? fields.map((field) => `        self.${field} = ${pythonClassFieldAssignment(field)}`)
        : [];
      const renderedMethods = renderClassMethodsForLanguage(methods, fields, lowerExt);
      return [
        `class ${className}:`,
        ...(constructorSignature ? [constructorSignature] : []),
        ...constructorBody,
        ...(renderedMethods.length > 0 ? renderedMethods : ['    pass']),
      ].join('\n');
    }
  ...
  function minimalClassSnippet(className, lowerExt) {
    ...
    if (lowerExt === '.vim') {
      return `let s:${toSnakeCaseIdentifier(className)} = {}`;
    }
    if (lowerExt === '.sh') {
      return `create_${toSnakeCaseIdentifier(className)}() {\n  :\n}`;
    }
    if (lowerExt === '.c') {
      return `typedef struct ${className} {\n} ${className};`;
    }
    return '';
  }
  ...
  function generateInterfaceSnippet(interfaceIntentOrInstruction, ext) {
    const lowerExt = String(ext || '').toLowerCase();
    const interfaceIntent = resolveInterfaceIntent(interfaceIntentOrInstruction);
    const interfaceName = interfaceIntent.name;
    const fields = interfaceIntent.fields;
    const methods = interfaceIntent.methods;
  ...
  function resolveInterfaceIntent(interfaceIntentOrInstruction) {
    if (
      interfaceIntentOrInstruction
      && typeof interfaceIntentOrInstruction === 'object'
      && interfaceIntentOrInstruction.kind === 'interface'
    ) {
      return {
        name: interfaceIntentOrInstruction.name || 'Contrato',
        fields: Array.isArray(interfaceIntentOrInstruction.fields)
          ? interfaceIntentOrInstruction.fields
          : [],
        methods: Array.isArray(interfaceIntentOrInstruction.methods)
          ? interfaceIntentOrInstruction.methods
          : [],
      };
    }
    ...
    return {
      ...
      fields: inferStructureFields(interfaceIntentOrInstruction),
      methods: inferContractMethods(interfaceIntentOrInstruction),
    };
  }

  function generateStructSnippet(structIntentOrInstruction, ext) {
    const lowerExt = String(ext || '').toLowerCase();
    const structIntent = resolveStructIntent(structIntentOrInstruction);
    const structName = structIntent.name;
    const fields = structIntent.fields;
  ...
  function resolveStructIntent(structIntentOrInstruction) {
    if (
      structIntentOrInstruction
      && typeof structIntentOrInstruction === 'object'
      && structIntentOrInstruction.kind === 'struct'
    ) {
      return {
        name: structIntentOrInstruction.name || 'Registro',
        fields: Array.isArray(structIntentOrInstruction.fields)
          ? structIntentOrInstruction.fields
          : [],
      };
    }
    ...
    return {
      ...
      fields: inferStructureFields(structIntentOrInstruction),
    };
  }

  function generateModuleSnippet(moduleIntentOrInstruction, ext) {
    const lowerExt = String(ext || '').toLowerCase();
    const moduleIntent = resolveModuleIntent(moduleIntentOrInstruction);
    const moduleName = moduleIntent.name;
    const functions = moduleIntent.methods;
    ...
    if (['.ts', '.tsx'].includes(lowerExt)) {
      if (!hasFunctions) {
        return `export const ${moduleName} = {} as const;`;
      }
      return [
        `export const ${moduleName} = {`,
        ...functions.map((fnName, index) => [
    ...
    if (lowerExt === '.c') {
      const snakeName = toSnakeCaseIdentifier(moduleName);
      if (!hasFunctions) {
        return `static void ${snakeName}_main(void) {\n  return;\n}`;
      }
      return functions
        .map((fnName) => [
  ...
  function resolveModuleIntent(moduleIntentOrInstruction) {
    if (
      moduleIntentOrInstruction
      && typeof moduleIntentOrInstruction === 'object'
      && moduleIntentOrInstruction.kind === 'module'
    ) {
      return {
        name: moduleIntentOrInstruction.name || 'CoreModule',
        methods: Array.isArray(moduleIntentOrInstruction.methods)
          ? moduleIntentOrInstruction.methods
          : [],
      };
    }
    ...
    return {
      ...
      methods: inferModuleFunctions(moduleIntentOrInstruction),
    };
  }
  ...
  function structuredIdentityExistsInText(content, semanticIdentity, ext, snippet) {
    ...
    if (semanticIdentity.kind === 'module') {
      if (['.js', '.jsx', '.mjs', '.cjs', '.ts', '.tsx'].includes(lowerExt)) {
        ...
        patterns.push(new RegExp(`\\b(?:function!\\s+s:${escapedSnakeName}_|let\\s+s:${escapedSnakeName}\\s*=\\s*\\{)`));
      } else if (lowerExt === '.sh') {
        patterns.push(new RegExp(`\\b${escapedSnakeName}_(?:listar|main)\\s*\\(\\)`));
      } else if (lowerExt === '.c') {
        patterns.push(new RegExp(`\\b${escapedSnakeName}_(?:listar|main)\\s*\\(`));
      }
    }
