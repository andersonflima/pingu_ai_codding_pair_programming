...
function createTerminalTaskTools(deps) {
  ...
  function extractExplicitTerminalCommand(instruction) {
    ...
    const directCommandPattern = /^(?:git|npm|pnpm|yarn|bun|npx|node|python|python3|pytest|pip|pip3|go|cargo|mix|sh|bash|zsh|fish|make|docker|docker-compose|kubectl|helm|terraform|ansible|echo|printf|pwd|ls|cat|grep|rg)\b/i;
