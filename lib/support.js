...
function commentPrefix(ext) {
  if (['.js', '.jsx', '.ts', '.tsx', '.go', '.java', '.kts', '.kt', '.cs', '.c', '.cpp', '.h', '.hpp', '.rs', '.scala', '.swift'].includes(ext.toLowerCase())) {
    return '//';
  }
...
function isCommentLine(line, ext) {
  ...
  if (trimmed.startsWith('///') || trimmed.startsWith('//')) {
    return ['.js', '.jsx', '.ts', '.tsx', '.go', '.java', '.kts', '.kt', '.cs', '.c', '.cpp', '.h', '.hpp', '.rs', '.scala', '.swift'].includes(lowerExt);
  }
...
function matchAssignmentStatement(line, ext) {
  ...
  if (['.js', '.jsx', '.ts', '.tsx', '.mjs', '.cjs'].includes(lowerExt)) {
    match = cleaned.match(/^(?:const|let|var)\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.+?);?$/);
  } else if (['.py', '.rb', '.ex', '.exs'].includes(lowerExt)) {
...
function matchConditionalStatement(line, ext) {
  ...
  if (['.js', '.jsx', '.ts', '.tsx', '.go', '.rs', '.c', '.cpp', '.java', '.cs', '.kt', '.kts'].includes(lowerExt) && /^if\s*\(.+\)\s*\{?$/.test(cleaned)) {
    return cleaned;
  }
  if (['.py', '.rb'].includes(lowerExt) && /^if\s+.+:?$/.test(cleaned)) {
    return cleaned;
  }
  if (['.ex', '.exs'].includes(lowerExt) && /^(if|unless)\s+.+\s+do\b/.test(cleaned)) {
    return cleaned;
  }
...
function snippetTabs() {
  return '# Substitua tabs por dois espacos em branco para manter padrao do formatter';
}
