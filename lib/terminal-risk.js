...
const WORKSPACE_WRITE_PATTERNS = Object.freeze([
  ...
  },
  {
    ...
    pattern: /\b(?:npm|pnpm|yarn|bun)\s+(?:run\s+)?(test|build|format|lint|dev|start)\b|\bmix\s+(test|compile|format|run)\b|\bgo\s+(test|build|run)\b|\bcargo\s+(test|build|run|fmt|clippy)\b|\bpython\s+-m\s+(pytest|py_compile)\b|\bpytest\b|\bctest\b|\bmake\s+test\b/i,
  },
  {
