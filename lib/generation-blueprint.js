...
function createBlueprintTools(deps) {
  ...
  function buildContextBlueprintTasks(_lines, file, lineNumber, instruction) {
    ...
    if (strictAiContextBlueprint && !canUseAiContextResolution) {
      return [
        buildAiContextFailureIssue(
          file,
          lineNumber,
          'IA obrigatoria para consolidar o contexto do blueprint',
          'Garanta que OPENAI_API_KEY esteja configurada para o OpenAI Codex gerar o documento de contexto.',
        ),
      ];
    }
    ...
    const shouldResolveAiContext = canUseAiContextResolution
      && (strictAiContextBlueprint || isElixirBlueprintFile(file));
    ...
    if (shouldResolveAiContext) {
      ...
      if (!aiContext || !String(aiContext.snippet || '').trim()) {
        if (!strictAiContextBlueprint && !isElixirBlueprintFile(file)) {
          aiContextResolved = false;
        } else {
          const failureIssue = buildAiContextFailureIssue(
            file,
            lineNumber,
            strictAiContextBlueprint
              ? 'IA obrigatoria para consolidar o contexto do blueprint'
              : 'IA obrigatoria para consolidar contexto ativo em Elixir',
            strictAiContextBlueprint
              ? 'Garanta que OPENAI_API_KEY esteja configurada para o OpenAI Codex gerar o documento de contexto.'
              : 'Garanta que OPENAI_API_KEY esteja configurada para o OpenAI Codex gerar o documento de contexto ativo.',
          );
          return [
            ...(strictAiContextBlueprint ? [] : tasks),
            failureIssue,
          ];
        }
      } else {
