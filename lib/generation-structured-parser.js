...
function createStructuredIntentParser(helpers = {}) {
  ...
  function inferStructureFields(instruction, fallbackFields = []) {
  ...
  function hasRealtimeChatSemantics(instruction, methods = []) {
    const text = String(instruction || '').toLowerCase();
    return methods.includes('broadcast')
      && (
        /\b(?:chat|room|rooms|sala|salas|usuario|usuarios|user|users|realtime|tempo real|socket|websocket)\b/.test(text)
        || /\b(?:mesma room|same room)\b/.test(text)
      );
  }

  function inferClassName(instruction, methods = []) {
    const fallbackName = hasRealtimeChatSemantics(instruction, methods) ? 'RoomBroadcaster' : 'Servico';
    ...
    if (explicitNameMatch && explicitNameMatch[1] && !isInstructionNoiseToken(explicitNameMatch[1])) {
      const normalizedName = sanitizeNaturalIdentifier(explicitNameMatch[1]);
      if (hasRealtimeChatSemantics(instruction, methods) && ['broadcast', 'chat', 'room', 'rooms'].includes(normalizedName)) {
        return 'RoomBroadcaster';
      }
      return toPascalCaseIdentifier(explicitNameMatch[1], fallbackName);
    }
    return toPascalCaseIdentifier(fallbackName, fallbackName);
  }
  ...
  function inferClassMethods(instruction) {
    ...
    if (hasRealtimeChatSemantics(text, [...explicitMethods, ...semanticMethods])) {
      semanticMethods.push('join_room', 'leave_room');
    }
  ...
  function inferClassFields(instruction, methods = []) {
    ...
    if (hasRealtimeChatSemantics(text, methods) || methods.includes('broadcast') || /\busuarios?_conectados?_a_rooms?\b/i.test(text)) {
  ...
  function parseStructuredIntent(instruction) {
    ...
    if (/\b(class|classe)\b/.test(lower)) {
      ...
      return {
        kind: 'class',
        name: inferClassName(normalizedInstruction, methods),
